...
def GetIsoFile(filename: str):
    with start_action(
        action_type=f"Getting {filename}: Started at {time.strftime('%X')}",
        currentThread=threading.get_ident(),
    ) as action:
        resp = requests.api.get(ISO_FILE)
        if resp.status_code == 200:
            open((DOWNLOAD_FOLDER / filename).with_suffix(".iso"), "wb").write(
                resp.content
            )
        action.log(
            message_type=f"Completed getting {filename} at {time.strftime('%X')}",
...
def main():
    ...
    with start_action(action_type="Creating threads and starting threadss") as action:
        thread1 = Thread(
            target=preserve_context(GetIsoFile), kwargs={"filename": "iso1"}
        )
        thread2 = Thread(
            target=preserve_context(GetIsoFile), kwargs={"filename": "iso2"}
        )
        thread3 = Thread(
            target=preserve_context(GetIsoFile), kwargs={"filename": "iso3"}
        )
        action.log(
            message_type="Thread ids:",
            thread1=thread1.name,
            thread2=thread2.name,
            thread3=thread3.name,
        )
        thread1.start()
        thread2.start()
        thread3.start()
